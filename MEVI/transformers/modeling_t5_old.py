...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if decode_embedding and hierarchic_decode:
            ...
            self.decoder = self.decoder = HierarchicT5Stack(decoder_config, self.decode_embeddings,
                                                            depth=config.decode_depth)
        else:
            self.decoder = T5Stack(decoder_config, self.decode_embeddings)

        print("adaptor_decode", adaptor_decode)
        print("adaptor_efficient", self.adaptor_efficient)
        if adaptor_decode and not self.adaptor_efficient:
            ...
            self.adaptor = T5Stack(decoder_config, self.decode_embeddings)  # [batch_size, seq_len, emb_dim]
            ...
            self.adaptor = nn.TransformerDecoderLayer(d_model=config.d_model, nhead=8)
            self.adaptor_linear = nn.Linear(config.d_model, config.d_model * config.decode_vocab_size, bias=False)
        else:
            print('00')
            self.adaptor_embeddings = None
        ...
        if tie_decode_embedding:
            if adaptor_decode and self.adaptor_efficient:
    ...
    def forward(
        ...
        if self.adaptor and not self.adaptor_efficient:
            ...
            #adaptor_weight = self.adaptor_linear(adaptor_output).reshape(adaptor_output.shape[0], )
            #adaptor_weight = adaptor_output # shape [batch_size, sequence_length, model_dim, vocab_size]
            lm_head_weight = self.lm_head.weight.T.unsqueeze(0).unsqueeze(0) # [1, 1, config.d_model, vocab_size]
            #print("adaptor_weight", adaptor_weight.shape)
            #print("lm_head.weight", lm_head.shape)
            lm_head_weight = adaptor_weight + lm_head_weight # [batch_size, sequence_length, config.d_model, vocab_size]
            lm_logits = torch.matmul(sequence_output.unsqueeze(-2), lm_head_weight)  # batch matmul, [batch, seq, 1, d_model] * [batch, seq, d_model, vocab_size] = [batch, seq, 1, vocab_size]
            lm_logits = lm_logits.squeeze(-2)
            # Mean: tensor([[-1.8537, -1.9613, -0.5768,  0.8496,  0.0073, -0.4369, -1.0046,  1.4947,
            #           0.4125, -1.8342, -1.2133, -1.1161, -1.1182, -1.2993,  0.5234]],
            #        device='cuda:0', grad_fn=<VarMeanBackward0>)
            # Var: tensor([[368.4430, 430.7539, 313.8922, 430.1031, 320.1245, 309.1241, 383.9584,
            #          287.7882, 311.3695, 307.5513, 363.3800, 337.9222, 315.1111, 311.4227,
            #          324.2958]], device='cuda:0', grad_fn=<VarMeanBackward0>)
            # Mean: -0.01897280663251877, Var: 0.41168874502182007
            ...
            decoder_input_embedding = self.adaptor(decoder_input_embedding.transpose(0,1), encode_embedding.transpose(0,1), tgt_mask=mask).transpose(0,1)
            #print(mask)
            #decoder_input_embedding = decoder_input_embedding.unsqueeze(1) * mask.unsqueeze(0).unsqueeze(-1)    #[batch_size, seq_length, seq_length, config.d_model]
            #print(decoder_input_embedding)
            #decoder_input_embedding = torch.sum(decoder_input_embedding, axis=1)   #[batch_size, seq_length, config,d_model]
            adaptor_weight = self.adaptor_linear(decoder_input_embedding).reshape(decoder_input_embedding.shape[0], decoder_input_embedding.shape[1], self.model_dim, -1)
            lm_head_weight = adaptor_weight + lm_head_weight
            lm_logits = torch.matmul(sequence_output.unsqueeze(-2), lm_head_weight)
            lm_logits = lm_logits.squeeze(-2)
        else:
            lm_logits = self.lm_head(sequence_output)  # shape(batch_size, sequence_length, config.vocab_size)
            # Mean: tensor([[ 0.0900,  0.1265,  0.1006, -0.0539,  0.0218,  0.1385,  0.0842,  0.1218,
            #           0.0946, -0.0238,  0.0060,  0.0277,  0.0855, -0.0023, -0.0038]],
            #        device='cuda:0', grad_fn=<VarMeanBackward0>)
            # Var: tensor([[1.6567, 1.5932, 1.3747, 1.5771, 1.3934, 1.6039, 1.4858, 1.3899, 1.7298,
            #          1.4582, 1.3788, 1.4515, 1.5223, 1.5273, 1.4812]], device='cuda:0',
            #        grad_fn=<VarMeanBackward0>)
            # Mean: 0.10657303780317307, Var: 1.5350384712219238
        # if self.training:
        #     var, mean = torch.var_mean(lm_logits)
        #     print(f"Mean: {mean}")
        #     print(f"Var: {var}")
        ...
        if labels is not None:
            ...
            if self.Rdrop > 0:
                ...
                loss = orig_loss + self.Rdrop*kl_loss
            else:
        ...
        if self.Rdrop > 0 and labels is not None:
            return_result.orig_loss = orig_loss
            return_result.kl_loss = self.Rdrop*kl_loss
